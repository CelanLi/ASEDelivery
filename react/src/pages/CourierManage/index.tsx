...
const DelivererAPI = 'http://localhost:8089/v1/api/user/find/all';
const DelivererAddAPI = 'http://localhost:8089/v1/api/user/publish';
const DelivererRemoveAPI = 'http://localhost:8089/v1/api/user/remove';

// 查询list
const handleQueryList = async (query: object) => {
  const res = await request(DelivererAPI, {
    query,
  });
  return res;
};
...
const handleAdd = async () => {
  ...
  try {
    request(DelivererAPI, {
      method: 'post',
      data: {
        /* proForm里写入的数据,并且角色为deliverer */
      },
    })
    hide();
